...
int main() {
	const char *appname = "vk texture test";
	...
	std::vector<uint32_t> vTextureData;
	{
		for(int j = 0 ; j < TEX_HEIGHT; j++) {
			for(int i = 0 ; i < TEX_WIDTH; i++) {
				uint32_t temp = i ^ j;
				vTextureData.push_back(temp);
				vTextureData.push_back(temp);
				vTextureData.push_back(temp);
				vTextureData.push_back(temp);
			}
	...
	const size_t texture_size = vTextureData.size() * sizeof(uint32_t);
	...
	auto systemmemory_allocator = new MemoryAllocator(device, devicememoryprop, MaxGpuMemory, false);
