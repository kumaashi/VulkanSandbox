...
std::vector<VkPipelineShaderStageCreateInfo> CreatePipelineFromFile(VkDevice device, PipelineCreateInfo &info)
{
	const char *complier_name = "glslangValidator.exe";
	const char *filehead = info.headername;
	...
	auto ret = new Pipeline(device, info.vertexInputAttr, info.vertexSize, info.layout, info.renderPass, info.attachmentCount, shader_create_info);
	return ret;
}
...
int main() {
	...
	//Create Shader
	const char *complier_name = "glslangValidator.exe";
	pipeline = CreatePipeline("basic");
	pipeline_present = CreatePipeline("present");
	...
	while(win_proc_msg()) {
		...
		if(pipeline == nullptr) {
			pipeline = CreatePipelineFromFile(device, "basic", vertexInputAttr, sizeof(VertexFormat), );
			pipeline_present = CreatePipelineFromFile(
				device,
				"present",
				vertexInputAttr,
				sizeof(VertexFormat),
				pipelineLayout->Get(),
				render_pass_4->Get(),
				render_pass_4->GetAttachmentCount(),
				shader_create_info);
		}
		k += 0.01;
